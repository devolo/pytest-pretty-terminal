    ...
    if hasattr(pytest, "reporter") and getattr(item.config.option, "pretty", False):
        if when == "setup":
            if not step:
                ...
                pytest.reporter.tr.line("")
                pytest.reporter.tr.write_sep("-", title, bold=True)
                pytest.reporter.tr.write_line(inspect.cleandoc(item.obj.__doc__ or ""))
                pytest.reporter.tr.write_line("parameterization " + specs if specs else "")
            ...
            if not step:
                pytest.reporter.tr.write_sep("-", bold=True)
                fill = getattr(pytest.reporter.tr, "_tw").fullwidth - getattr(pytest.reporter.tr, "_width_of_current_line") - 1
